...
schtasks /CREATE /TN 'Enable Remoting' /SC WEEKLY ``
    ...
    /TR "C:\WINDOWS\system32\WindowsPowerShell\v1.0\powershell.exe -noprofile -command Enable-PsRemoting -Force" /F
...
Invoke-Command $computername { Get-WmiObject Win32_ComputerSystem } -Credential $credential